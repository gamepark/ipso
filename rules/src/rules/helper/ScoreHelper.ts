...
export class ScoreHelper extends MaterialRulesPart {
  ...
  getStars(player: number): number {
    return this.getPlayerCards(player)
      .map(card => numberCardData[card.id as NumberCard].stars)
  ...
  getLinePoints(player: number, lineY: number): number {
    ...
    if (!isAscending(lineCards.map(card => numberCardData[card.id as NumberCard].number))) return 0
    return lineCards.length * this.getPointsByCard(lineCards)
  }

  private getPointsByCard(lineCards: MaterialItem[]): number {
    let pointsByCard = 1
  ...
  private getPlayerCards(player: number): MaterialItem[] {
