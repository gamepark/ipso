...
import { OddOrEvenDialogContext, useOddOrEvenDialog } from './OddOrEvenContext'
import { TABLE_X_MAX, TABLE_X_MIN, TABLE_Y_MAX, TABLE_Y_MIN } from './panels/PanelPosition'
import { PlayerPanels } from './panels/PlayerPanels'

export function GameDisplay() {
  const margin = { top: 7, left: 0, right: 0, bottom: 0 }
  const { show, dismiss, reopen } = useOddOrEvenDialog()
  return (
    <OddOrEvenDialogContext.Provider value={reopen}>
      ...
      <OddOrEvenDialog open={show} onClose={dismiss} />
    </OddOrEvenDialogContext.Provider>
  )
}
